...
void write_mpiio_dump(mpiInfo &myMPI)
{
  ...
  int nRealx = myMPI.nRealx;
  int nRealy = myMPI.nRealy;
  int nTotalx = nRealx + 1;
  int nTotaly = nRealy + 1;
  int nPEx   = myMPI.nPEx;
  int nPEy   = myMPI.nPEy;
  int iPE    = myMPI.iPE;
  int jPE    = myMPI.jPE;
  ...
  printf("PE: %d, nRealx = %d, nRealy = %d, nPEx = %d, nPEy = %d, iPE = %d, jPE = %d\n",
         myMPI.myPE, nRealx, nRealy, nPEx, nPEy, iPE, jPE);
