...
int main(int argc, char *argv[])
{
   ...
   for (int count =  0 ; count < argc; ++count)
     {
       ...
       if ( !strcmp(argv[count],"-plt_out_path" ) ) plt_out_path = argv[count+1];
     }
  ...
  //  MESH.write_mpiio_dump(myMPI); 
  //  FatalError("Done with write_mpiio_dump"); 
   ...
   for ( double time = tStart ; time <= tEnd ; time += dt )
       ...
       if ( timeSinceLastPlot >= tPlot )
	 {
	   ...
	   MESH.plot( "phi"  , MESH.phi ,     count , myMPI      , plt_out_path );
     if (myMPI.myPE == 0) printf("PE: %d, phi size = %d, phiNew size = %d", myMPI.myPE, \
                                  MESH.phi.size(), MESH.phiNew.size());
	   timeSinceLastPlot = 0.;
	   
	   MPI_Barrier(MPI_COMM_WORLD);
	   MESH.write_mpiio_dump(myMPI);             // <-----  TO-DO:  Make special note of this
